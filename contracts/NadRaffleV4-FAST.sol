...
contract NadRaffleV4Fast is ReentrancyGuard, Ownable2Step, Pausable {
    ...
    // ✅ FAST: Reduced reveal window for Monad's 0.5s block time
    mapping(uint256 => RandomnessCommit) public randomnessCommits;
    mapping(uint256 => bytes32) public raffleSeeds;
    uint256 public constant REVEAL_WINDOW = 2 minutes; // ⚡ FAST: 2 minutes instead of 1 hour
    
    // ✅ Enhanced security features (unchanged)
    uint256 public constant MAX_TITLE_LENGTH = 100;
    uint256 public constant MAX_DESCRIPTION_LENGTH = 500;
    uint256 public constant MAX_TICKETS = 100000;
    uint256 public constant MIN_TICKET_PRICE = 1000;
    ...
    // ✅ Front-running protection (unchanged)
    ...
    // ✅ Create raffle function (unchanged from Ultra-Secure)
    function createRaffle(
        ...
        // ✅ Generate secure seed for this raffle
    ...
    // ✅ Purchase tickets function (unchanged)
        ...
        notEmergencyPaused
    {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            raffleTickets[raffleId].push(Ticket({
                raffleId: raffleId,
                buyer: msg.sender,
                ticketNumber: raffle.ticketsSold + i,
                purchaseTime: block.timestamp,
                randomSeed: randomSeed
            }));
            
            emit TicketPurchased(raffleId, msg.sender, raffle.ticketsSold + i, raffle.ticketPaymentToken, raffle.ticketPrice, randomSeed);
        }
        ...
        // ✅ Auto-commit randomness when sold out
        if (raffle.ticketsSold >= raffle.maxTickets && randomnessCommits[raffleId].commitment == bytes32(0)) {
            _commitRandomnessForRaffle(raffleId);
        }
    }

    // ✅ FAST randomness functions with 2-minute window
    function _commitRandomnessForRaffle(uint256 raffleId) internal {
        ...
        randomnessCommits[raffleId] = RandomnessCommit({
            ...
            revealed: false,
            revealDeadline: block.timestamp + REVEAL_WINDOW // ⚡ 2 minutes instead of 1 hour
        });
