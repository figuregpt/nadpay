...
export default function PaymentContent() {
  ...
  // Contract hooks
  const { data: contractLink, isLoading: loadingLink, error: contractError } = usePaymentLink(parseInt(linkId) || -1);
  const { data: userPurchaseCount } = useUserPurchaseCount(parseInt(linkId) || -1, address);
  ...
  const paymentLink = contractLink ? {
    ...
    purchases: [],
    createdAt: new Date(Number(contractLink.createdAt) * 1000).toISOString(),
  } : null;
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      await purchase(parseInt(linkId), quantity, totalPrice.toString());
    } catch (error) {
